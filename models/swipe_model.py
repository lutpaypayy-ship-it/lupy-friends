...
class SwipeModel:
    ...
    @classmethod
    def ambil_user_acak(cls, user_id, genre_filter="Semua"):
        """Fitur 5: Mengambil 1 user acak yang BELUM PERNAH di-swipe dengan filter genre"""
        conn = get_db_connection()
        if not conn: return None
        
        cursor = conn.cursor(dictionary=True)
        
        # Query dasar menggunakan kolom asli lu (swiper_id & swipee_id)
        query = """
            SELECT * FROM users 
            WHERE id != %s 
        ...
        # Tambahkan filter genre musik jika user memilih selain "Semua"
        if genre_filter != "Semua":
            query += " AND genre_musik LIKE %s"
            params.append(f"%{genre_filter}%")
            
        # Urutkan secara acak dan batasi hanya mengambil 1 data
